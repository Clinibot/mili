...
export default function DocsPage() {
    ...
    const renderIcon = (item: DocItem) => {
        if (item.type === 'url') return <LinkIcon className="text-blue-500" size={20} />;
        if (item.file_name?.match(/\.(jpg|jpeg|png|gif)$/i)) return <ImageIcon className="text-purple-500" size={20} />;
        if (item.file_name?.match(/\.(pdf)$/i)) return <FileText className="text-red-500" size={20} />;
        ...
        <DashboardLayout>
            <div className="max-w-6xl mx-auto space-y-8">
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-4xl font-bold text-slate-900 tracking-tight">Documentación Compartida</h1>
                        <p className="text-slate-500 mt-2">Recursos y guías para Sonia y Mili.</p>
                    </div>
                    <button
                        onClick={() => setIsAddOpen(true)}
                        className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-2xl font-bold flex items-center gap-2 shadow-lg shadow-blue-600/20 transition-all active:scale-95"
                    >
                        <Plus size={20} />
                        Añadir Recurso
                    </button>
                ...
                {loading ? (
                    ...
                    <div className="bg-white border border-dashed border-slate-200 rounded-3xl p-16 text-center">
                        ...
                        <h3 className="text-xl font-bold text-slate-800">No hay documentos aún</h3>
                        <p className="text-slate-500 mt-1">Empieza por subir una URL o un documento importante.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {items.map((item) => (
                            <Card key={item.id} className="group border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 rounded-3xl overflow-hidden bg-white">
                                <CardContent className="p-6">
                                    ...
                                    <p className="text-sm text-slate-400 mb-6 truncate max-w-full">
                                    ...
                                    <div className="flex gap-2">
                                        <a
                                            ...
                                            className="flex-1 bg-slate-50 hover:bg-blue-50 text-slate-600 hover:text-blue-600 py-2.5 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2"
                                        >
                ...
                {/* Add Modal Placeholder (Manually built for speed/control) */}
                {isAddOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
                        <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden border border-slate-100">
                            <div className="p-6 border-b border-slate-50 flex justify-between items-center">
                                <h2 className="text-xl font-bold text-slate-900">Añadir Recurso</h2>
                            ...
                            <form onSubmit={handleAdd} className="p-6 space-y-5">
                                ...
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-slate-700">Tipo de Recurso</label>
                                    <div className="flex bg-slate-50 p-1 rounded-xl">
                                        <button
                                            ...
                                            className={cn("flex-1 py-2 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2",
                                                docType === 'url' ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-800")}
                                        >
                                            <LinkIcon size={16} />
                                            Enlace URL
                                        </button>
                                        <button
                                            ...
                                            className={cn("flex-1 py-2 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2",
                                                docType === 'file' ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-800")}
                                        >
                                            <Upload size={16} />
                                            Documento
                                        </button>
                                ...
                                {docType === 'url' ? (
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div
                                            ...
                                            className="cursor-pointer group border-2 border-dashed border-slate-200 rounded-2xl p-8 text-center hover:border-blue-400 hover:bg-blue-50 transition-all"
                                        >
                                            <input
                                                type="file"
                                                ref={fileInputRef}
                                ...
                                <button
                                    disabled={uploading}
                                    className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-bold py-3 rounded-2xl shadow-lg shadow-blue-600/20 transition-all flex items-center justify-center gap-2"
                                >
